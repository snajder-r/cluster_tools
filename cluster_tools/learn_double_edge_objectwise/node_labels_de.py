...
class NodeSemanticLabelsBase(luigi.Task):
    """ Edge labels base class
    """

    task_name = 'node_labels_de'
...
class  NodeSemanticLabelsLocal(NodeSemanticLabelsBase, LocalTask):
    """ EdgeLabels on local machine
    """
    pass


class  NodeSemanticLabelsSlurm(NodeSemanticLabelsBase, SlurmTask):
    """ EdgeLabels on slurm cluster
    """
    pass


class  NodeSemanticLabelsLSF(NodeSemanticLabelsBase, LSFTask):
...
# TODO parallelize
def node_labels(job_id, config_path):
    ...
    node_semantic = np.zeros(int(np.max(node_ids)), dtype=np.uint32)
    for i, node in enumerate(list(node_ids),0):
        ...
        # first -1 is because i dont care about ignore label 0, second -1 is because i transform classes 1,2,3 to 0,1,2
        if node >= 1:
            print(node-1)
            node_semantic[int(node-1)] = sem_label-1 
        
    
    chunks = (min(262144, node_semantic.shape[0]),)
