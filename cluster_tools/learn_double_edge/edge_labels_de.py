...
def edge_labels(job_id, config_path):
    ...
    # DEBUG
    with vu.file_reader(gt_node_labels_path, 'a') as f:
        f.require_dataset('debug_sem_labels', shape=node_semantic.shape, compression='gzip', dtype='uint8')
        f['debug_sem_labels'][:] = node_semantic
    
    lu = uv_ids[:, 0]
    lv = uv_ids[:, 1]
    
    # double edges
    double_edge = np.logical_or(
                      np.logical_and(node_semantic[lu] == semantic_label_bg, node_semantic[lv] == semantic_label_fc),
                      np.logical_and(node_semantic[lv] == semantic_label_bg, node_semantic[lu] == semantic_label_fc)).astype('uint8')
    
    print("Got %d edges that are double " % sum(double_edge))
    
    n_edges = len(double_edge)
    chunks = (min(262144, n_edges),)
    with vu.file_reader(output_path) as f:
        f.require_dataset(output_key, shape=double_edge.shape, chunks=chunks, compression='gzip', dtype='uint8')
        f[output_key][:] = double_edge
        f.attrs['double_edges'] = int(double_edge.sum())
